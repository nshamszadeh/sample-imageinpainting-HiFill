...
out = cv2.VideoWriter('../../final.avi', codec, 30.0, size)
frame = 1
...
with tf.Graph().as_default():
  ...
  with tf.Session() as sess:
    ...
    while(mask_capture.isOpened() and input_capture.isOpened()):
        ret, image = input_capture.read()
        mask_ret, mask = mask_capture.read()

        if ret and mask_ret:
            assert image.shape == mask.shape
            raw_frame = image
            raw_mask = mask
            inpainted = inpaint(raw_frame, raw_mask, sess, inpainted_512_node,
                                attention_node, mask_512_node, image_ph, mask_ph, args.multiple)
            out.write(inpainted)
